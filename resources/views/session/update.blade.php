            ...
            <div class="col-md-8">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('course.session.update', ['course'=>$course, 'session'=>$session]) }}"
                              method="post">
                            @csrf
                            @method('PUT')
                                        ...
                                        <tr>
                                            <td>
                                                <input type='checkbox' name='students[]'
                                                       value='{{ $student->id }}' {{ $session->students()->where('student_id', $student->id)->where('presence', 1)->count()?'checked':'' }}/>
                                            </td>
                                            <td>{{ $student->fname }}</td>
                                            <td>{{ $student->lname }}</td>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" rows="5" name="description" id="description">{{ old('description', $session->description) }}</textarea>
                            </div>
